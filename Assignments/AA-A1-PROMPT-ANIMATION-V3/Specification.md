...
The target scene contains exactly two faceless humanoid figures interacting with exactly one shared sphere in front of a large black-and-ivory Taijitu.
The Taijitu should remain visually stable while the two figures perform a coordinated sequence of reaching for, lifting, passing, receiving, crouching with, and returning the same sphere toward the opening arrangement.
...
| Reference | Purpose |
| --- | --- |
| [Motion reference](assets/motion-reference.gif) | Defines the composition, two-character action sequence, single-sphere movement and handoffs, timing, and loop behaviour. |
...
For object counting, the final animation must contain exactly two humanoid figures and one independent shared sphere at every point in the sequence.
The figures' spherical heads are body parts and do not count as the shared sphere.
If the low-resolution motion reference appears ambiguous, this written count takes precedence.
...
- retain exactly two distinguishable humanoid figures throughout the sequence;
- retain exactly one independent shared sphere throughout the sequence;
- retain the large Taijitu's geometry, orientation, framing, and black-and-ivory material;
- preserve the shared sphere's handoffs and the broad pose sequence of the motion reference;
- keep the same sphere's identity, size, material, and continuity across every handoff;
- maintain coherent anatomy, foot contact, hand-to-sphere contact, lighting, and shadows across frames;
- avoid visible morphing, duplicated limbs, identity swaps, sphere duplication, background drift, object flicker, and abrupt temporal jumps.
...
At each checkpoint, inspect character count and placement, pose and action, sphere count, identity, and contact, Taijitu stability, material and shadow continuity, and visible artefacts.
