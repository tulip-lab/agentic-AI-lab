...
目标场景包含恰好两名无面部特征的人形角色。
他们在一个大型黑白太极图前，与恰好一个共享球体进行互动。
太极图应在动画中保持稳定；两名角色应围绕同一个球体协调完成伸手、举起、传递、接取、下蹲，并逐步回到接近初始构图的一组动作。
...
| 参考材料 | 用途 |
| --- | --- |
| [动作参考动画](assets/motion-reference.gif) | 定义构图、两名角色的动作顺序、单一球体的移动与传递、时间节奏和循环方式。 |
...
对人物和物体进行计数时，整段最终动画在任何时刻都必须恰好包含两名人形角色和一个独立的共享球体。
角色的球形头部属于身体结构，不计作该共享球体。
如果低分辨率参考动画存在视觉歧义，以本条书面数量要求为准。
...
- 分辨率不低于 720 x 720；
- 帧率不低于每秒 12 帧；
- 在整个动画中恰好保留两名可区分的人形角色；
- 在整个动画中恰好保留一个独立的共享球体；
- 保留大型太极图的几何形态、方向、画面位置以及黑白材质；
- 保留参考动画中该共享球体的传递过程和整体动作顺序；
- 在每次传递中保持同一球体的身份、尺寸、材质和时间连续性；
- 在不同帧之间保持合理的人体结构、脚部接地、手与球体接触、光照和阴影；
- 避免明显的人物变形、肢体重复、角色身份互换、球体复制、背景漂移、物体闪烁和突兀的时间跳变。
...
在每个检查点观察角色数量与位置、姿势与动作、球体数量、身份与接触、太极图稳定性、材质和阴影连续性，以及可见瑕疵。
